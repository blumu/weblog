...
module Blog = 
  ...
  let LoadModel(tagRenames, transformer, (root:string), blog, (properties:(string*string) seq)) = 
    ...
    { Posts = posts
      ...
      MonthlyPosts = 
        query { for p in posts do
                ...
                select (year, us.DateTimeFormat.GetMonthName(month), g :> seq<_>) }
        |> Array.ofSeq
      Root = root.Replace('\\', '/') 
  ...
  let TransformFile template hasHeader (razor:FsBlogLib.Razor) prefix current target = 
    let html =
      ...
      | (".fsx" | ".md") as ext ->
          ...
          let parsed = 
            if ext = ".fsx" then
              Literate.ParseScriptString(content)
            else
              Literate.ParseMarkdownString(content)
          let parsed = parsed.With(List.map Transforms.generateSubheadings parsed.Paragraphs)
          Literate.ProcessDocument(parsed, html.FileName, template, OutputKind.Html, ?prefix=prefix, ?lineNumbers = Some false)
          let processed = File.ReadAllText(html.FileName)
          EnsureDirectory(Path.GetDirectoryName(target))
          razor.ProcessString(header + processed, html.FileName)
