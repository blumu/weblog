...
module BlogPosts = 
  ...
  type BlogHeader = 
    { Title : string
      ...
      Url : string
      EntryId : string option
      LegacyViews : int option
      Tags : seq<string> }
  ...
  let ParseBlogHeader renameTag (blog:string) =
    ...
    fun (file:string, header:string, abstr) ->
      ...
      let optionify f x =
        ...
        | false, _ -> None
        | true, v -> Some v
      let tryGet = optionify lookup.TryGetValue
      let tryParseint =
        optionify lookup.TryGetValue
        >> (Option.bind (optionify System.Int32.TryParse))
      ...
      try
        { Title = lookup.["Title"]
          ...
          EntryId = tryGet "EntryId"
          LegacyViews = tryParseint "LegacyViews"
        }
